...
int mkdir (const char *driver, const char *destDirPath){
    ...
    if (destDirPath[length - 1] == '/') {  //just to del this
        cond = 1;
...
int cp (const char *driver, const char *srcFilePath, const char *destFilePath) {
    ...
    /*
    if (srcFilePath[0] != '/' || srcFilePath[length - 1] == '/') {
        printf("Incorrect source file path:%s.\n",srcFilePath);
        fclose(file);
        return -1;
    }*/
